...
can actually be forecast, and what it costs when it cannot.
...
| equal weight | 19.4% | 15.0% | 0.861 | -32.4% | -2.1% | 1.000 | n/a |
...
| **Portfolio** equal-wt / min-vol / ERC | models passing 1 / 1 / **2**, so the ERC stream is the most forecastable |
...
target traces out a clean risk/return frontier: 4x the leverage for 4x the CAGR and
...
in the sidebar forbids network access entirely, which is also how the tests run.
...
`test_es_forecast_is_out_of_sample` is the tail-risk equivalent. Truncating the input
...
`test_a_failing_risk_model_overshoots_the_target_by_more` is the integration in one
...
`test_best_of_many_random_strategies_fails_deflation` runs forty pure-noise strategies,
